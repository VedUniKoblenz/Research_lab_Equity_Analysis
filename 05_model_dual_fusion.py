...
from config import compute_vol_series, CONTEXT_LEN, TRAIN_SPLIT_DATE, CHRONOS_MODEL, RAW_PRICES_DIR, PROC_EMBED_DIR, RESULTS_DIR, TICKERS, N_SEC_PCA, HORIZON, THRESHOLD_MULTIPLIER_DUAL, triple_barrier_label_v2, calculate_financial_metrics
...
def main():
    ...
    for ticker in TICKERS:
        ...
        for i in range(start_idx, len(df) - HORIZON):
            ...
            # Normalize the SEC PCA vector to a unit scale, then apply a fixed signal factor.
            sec_std = np.std(sec_vec) + 1e-6
            norm_sec = (sec_vec / sec_std) * 0.10
            ...
            # 4. Prediction: Forecast HORIZON days ahead
            ...
            median_path = full_path[-HORIZON:] # Extract only the future HORIZON-day window
            
            # RMSE
            actual_path = df.iloc[i+1:i+1+HORIZON][close_col].values
            rmses.append(np.sqrt(np.mean((median_path - actual_path)**2)))
            
            yt = triple_barrier_label_v2(actual_path, row[close_col], row['vol'], THRESHOLD_MULTIPLIER_DUAL, horizon=HORIZON)
            fwd_ret = float((median_path[-1] / row[close_col]) - 1)
            
            # Extreme Conviction Gating: 2.0x vol
            threshold = row['vol'] * np.sqrt(HORIZON) * THRESHOLD_MULTIPLIER_DUAL
