...
def main():
    ...
    print("\n" + "=" * 60)
    print("MODEL: PRICE ONLY")
    print("=" * 60)

    engine = ForwardTestEngine(use_dual_fusion=False)
    results = engine.run()
    ...
    if per_ticker_metrics is not None and not per_ticker_metrics.empty:
        ...
        per_ticker_df.to_csv(
            RESULTS_DIR / "price_only_per_ticker.csv", index=False
        )
    ...
    if portfolio_returns is not None and len(portfolio_returns) > 0:
        portfolio_returns.to_csv(
            RESULTS_DIR / "price_only_returns.csv"
        )
    ...
    summary.to_csv(RESULTS_DIR / "price_only_summary.csv", index=False)
