...
def parse_html_to_text(html_content):
    ...
    # 100k characters ensures we see the important sections at the end
    return soup.get_text(separator=" ", strip=True)[:100000] 

def summarize_filing(text, ticker):
    # RESTORED HIGH-QUALITY ALPHA PROMPT
    prompt = (
    ...
    for ticker, filing_dir in all_filing_dirs:
        ...
        if summary:
            with open(summary_file, 'w', encoding='utf-8') as f:
                f.write(summary)
            succeeded += 1
            # 5-second delay to keep you safe on the 15 RPM free tier
            time.sleep(5)
        else:
            failed += 1
